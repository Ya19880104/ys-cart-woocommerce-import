...
1. 開啟 `YS Plugin > YS CART WC 匯入`。
2. 建議先在「SQL 備份」區塊點「建立 SQL 備份」。
3. 在「匯入到 YS CART」區塊選擇 ZIP 套件。
4. 選擇資料類型：「客戶」、「商品」或「訂單」。
5. 點「上傳並匯入」。
6. 上傳成功後會顯示 manifest，並在「工作狀態」新增 `import/<entity>` job。
7. 介面會自動執行批次；需要手動重試時，可點「執行下一批」。
8. 若狀態還不是 `completed`，繼續使用「自動執行」或「執行下一批」，直到完成。
...
- 備份檔會存放在 `wp-content/uploads/ys-cart-wc-import/backups`。
- 備份列表可下載或刪除舊檔。
- 還原前會要求輸入 `RESTORE`，避免誤按覆蓋目前資料。
- 還原功能只應用於匯入前剛建立的備份；正式站建議仍保留主機層級備份。
